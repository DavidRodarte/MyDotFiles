...
function handleHoverRequest(document, params) {
    ...
    const foundBlock = (0, util_1.getModelOrEnumBlock)(word, lines);
    if (!foundBlock) {
        return;
    }
...
function prismaFmtCompletions(params, document) {
    const text = document.getText((0, util_1.fullDocumentRange)(document));
    const completionList = (0, textDocumentCompletion_1.default)(text, params);
...
function localCompletions(params, document) {
    ...
    const symbolBeforePosition = (0, completions_1.getSymbolBeforePosition)(document, position);
    ...
    if ((context === null || context === void 0 ? void 0 : context.triggerKind) === vscode_languageserver_1.CompletionTriggerKind.TriggerCharacter) {
        switch (context.triggerCharacter) {
            case '@':
                if (!(0, completions_1.positionIsAfterFieldAndType)(position, document, wordsBeforePosition)) {
                    return;
                }
                return (0, completions_1.getSuggestionForFieldAttribute)(foundBlock, (0, util_1.getCurrentLine)(document, position.line), lines, wordsBeforePosition, document);
            case '"':
                return (0, completions_1.getSuggestionForSupportedFields)(foundBlock.type, lines[position.line], currentLineUntrimmed, position, lines);
            case '.':
                return (0, completions_1.getSuggestionForNativeTypes)(foundBlock, wordsBeforePosition, document, lines);
        }
    }
    switch (foundBlock.type) {
        case 'model':
            // check if inside attribute
            if ((0, completions_1.isInsideAttribute)(currentLineUntrimmed, position, '()')) {
                return (0, completions_1.getSuggestionsForInsideRoundBrackets)(currentLineUntrimmed, lines, document, position, foundBlock);
            }
            // check if type
            if (!(0, completions_1.positionIsAfterFieldAndType)(position, document, wordsBeforePosition)) {
                return (0, completions_1.getSuggestionsForTypes)(foundBlock, lines, position, currentLineUntrimmed);
            }
            return (0, completions_1.getSuggestionForFieldAttribute)(foundBlock, lines[position.line], lines, wordsBeforePosition, document);
        case 'datasource':
        case 'generator':
            if (wordsBeforePosition.length === 1 &&
                symbolBeforePositionIsWhiteSpace) {
 ...
 */
function handleCompletionRequest(params, document) {
...
function handleRenameRequest(params, document) {
    ...
    const isModelRename = (0, renameUtil_1.isModelName)(params.position, currentBlock, lines, document);
    const isEnumRename = (0, renameUtil_1.isEnumName)(params.position, currentBlock, lines, document);
    ...
    if (isModelRename ||
        ...
        isEnumValueRename ||
        isValidFieldRename) {
        const currentName = (0, renameUtil_1.extractCurrentName)(currentLine, isModelRename || isEnumRename, isEnumValueRename, isValidFieldRename, document, params.position);
        ...
        if (!isRelationFieldRename &&
            !(0, renameUtil_1.mapExistsAlready)(lineOfDefinition, lines, blockOfDefinition, isModelRename || isEnumRename)) {
            // add map attribute
            edits.push((0, renameUtil_1.insertMapAttribute)(currentName, position, blockOfDefinition, isModelRename || isEnumRename));
        }
        // rename references
        if (isModelRename || isEnumRename) {
        ...
        (0, renameUtil_1.printLogMessage)(currentName, params.newName, isEnumRename, isModelRename, isValidFieldRename, isEnumValueRename);
        return {
            changes: {
                [document.uri]: edits,
            },
...
function handleCodeActions(params, document) {
    ...
    return (0, codeActionProvider_1.quickFix)(document, params);
}
